...
const ModeratorRegisterStage2 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`/api/auth/setup-moderator-password?token=${token}`, { password: password, login: login });
      setStatus('success');
      setTimeout(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-[#F5F5F5] p-8 rounded-[40px] shadow-lg border-[3px] border-[#69DC9E] w-full max-w-md text-center">
          <h2 className="text-2xl font-bold mb-4">Konto Zostalo stworzone!</h2>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-[#F5F5F5] p-8 rounded-[40px] shadow-lg border-[3px] border-[#69DC9E] w-full max-w-md">
        <h2 className="text-2xl font-bold mb-6 text-center">Nadaj hasło i login do twojego konta</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            >
              {status === 'submitting' ? 'Ustawianie...' : 'Ustaw hasło'}
            </button>
...
export default ModeratorRegisterStage2;